...
fun BookingDetailScreen(
            ...
            when (event) {
                ...
                is BookingDetailEvent.BookingCanceled -> {
                    scope.launch {
                        val message =
                            if (event.wasPaid)
                                "Reserva cancelada. Reembolso de ${event.refundAmount} realizado a tu tarjeta."
                            else "Reserva cancelada correctamente"
                        snackbarHostState.showSnackbar(message)
                    }
                }

                is BookingDetailEvent.ExtendPaymentCanceled -> {
                    showExtendPaymentCanceledDialog = true
                }

                is BookingDetailEvent.ExtendPaymentCompleted -> {
    ...
    if (state.showExtendDialog) {
        ExtendBookingDialog(
            initialDateMillis = state.extendInitialDateMillis,
    ...
    if (state.showExtendPaymentDialog) {
        SimulatedExtendPaymentDialog(
            amount = state.formattedPendingExtendAdditionalPrice,
            isLoading = state.isExtending,
            onDismiss = viewModel::cancelExtendPayment,
            onConfirm = viewModel::confirmExtendPayment,
        )
    }

    if (showExtendPaymentCanceledDialog) {
        AlertDialog(
            onDismissRequest = { showExtendPaymentCanceledDialog = false },
            title = { Text("Pago cancelado") },
            text = { Text("Se canceló el pago de la extensión.") },
            confirmButton = {
                TextButton(onClick = { showExtendPaymentCanceledDialog = false }) {
                    Text("Aceptar")
                }
            },
        )
    ...
    if (showExtendSuccessDialog) {
        AlertDialog(
            onDismissRequest = { showExtendSuccessDialog = false },
            title = { Text("¡Extensión exitosa!") },
            text = {
                Text(
                    "Tu estancia ha sido extendida correctamente. La nueva fecha de checkout es ${state.checkOutLabel}."
                )
            },
            confirmButton = {
                TextButton(onClick = { showExtendSuccessDialog = false }) {
                    Text("Aceptar")
                }
            },
        )
...
private fun BookingDetailContent(
    ...
    Column(
        ...
        if (state.isCanceled && booking.isPaid) {
            Card(
                ...
                Text(
                    text = "Reembolso realizado: ${state.formattedTotalPrice} a tu tarjeta.",
...
private fun ExtendBookingDialog(
    ...
    val datePickerState = rememberDatePickerState(initialSelectedDateMillis = initialDateMillis ?: currentEndDateMillis)
...
@Composable
private fun SimulatedExtendPaymentDialog(
    amount: String,
    isLoading: Boolean,
    onDismiss: () -> Unit,
    onConfirm: () -> Unit,
    ...
    AlertDialog(
        onDismissRequest = onDismiss,
        title = { Text(text = "Simular pago de extensión") },
        text = {
            Text(
                text =
                    "Debes confirmar el pago del excedente ($amount) para extender la estancia.",
                style = MaterialTheme.typography.bodyMedium,
            )
        },
        confirmButton = {
